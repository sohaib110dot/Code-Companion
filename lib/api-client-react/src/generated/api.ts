 ...
 */
export const getConvertVideoUrl = () => {
  return `/api/convert`;
};
...
export const convertVideo = async (
  ...
  return customFetch<ConvertResponse>(getConvertVideoUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
...
export const getConvertVideoMutationOptions = <
  TError = ErrorType<ErrorResponse>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof convertVideo>>,
    TError,
  ...
  TContext
> => {
  const mutationKey = ["convertVideo"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
  ...
  > = (props) => {
    ...
    return convertVideo(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type ConvertVideoMutationResult = NonNullable<
  Awaited<ReturnType<typeof convertVideo>>
>;
export type ConvertVideoMutationBody = BodyType<ConvertRequest>;
export type ConvertVideoMutationError = ErrorType<ErrorResponse>;
...
>(options?: {
  ...
  TContext
> => {
  return useMutation(getConvertVideoMutationOptions(options));
};
